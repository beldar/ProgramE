...
if ($_POST["input"]!="") {
  $f=new xmlrpcmsg('programe.getResponse', array(new xmlrpcval($_POST["input"], "string"),
                                              new xmlrpcval($_POST["myid"], "string"),
											  new xmlrpcval($_POST["botname"], "string")
                                             )
                  );
...
//  print "<pre>" . htmlentities($f->serialize()) . "</pre>\n";
  ...
  if (!$r->faultCode()) {
	print "Program E responds to:<br>". $_POST["input"] . "<br>with:<br>" . $theresponse->scalarval() . "<BR>";
  } else {
	print "Fault: ";
